...
class TestUserEndpoints:
    ...
    def test_validacion_formato_dni(self, api_client, create_user):
        """Test que verifica la validación del formato DNI"""
        api_client.force_authenticate(user=create_user)
        url = reverse('app:user-detail', args=[create_user.id])
        ...
        for invalid_dni in invalid_dnis:
            response = api_client.patch(url, data={"dni": invalid_dni})
            assert response.status_code == status.HTTP_400_BAD_REQUEST

    def test_actualizacion_perfil_completo(self, api_client, create_user):
        """Test que verifica que un usuario puede completar su perfil"""
        api_client.force_authenticate(user=create_user)
        url = reverse('app:user-detail', args=[create_user.id])
        ...
        response = api_client.patch(url, data=complete_profile_data)
        assert response.status_code == status.HTTP_200_OK
        create_user.refresh_from_db()
        assert create_user.is_profile_completed()
